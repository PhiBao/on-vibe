...
export async function GET(request: Request) {
  ...
  try {
    // Fetch candles, orderbook, and market info in parallel
    const [candlesRes, bookRes] = await Promise.all([
      fetch(`${PHOENIX_API}/v1/candles/${symbol}?timeframe=1h`, { next: { revalidate: 30 } }),
      fetch(`${PHOENIX_API}/v1/orderbook/${symbol}`, { next: { revalidate: 5 } }),
    ]);

    const candles = candlesRes.ok ? await candlesRes.json() : [];
    const book = bookRes.ok ? await bookRes.json() : {};

    // Compute basic indicators on server
