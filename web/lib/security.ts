...
export class SecurityAuditor {
  ...
  constructor(config: { maxOrderSize?: number; maxDailyVolume?: number } = {}) {
    ...
    this.suspiciousPatterns = [
      /0x0{10,}/i, // Suspicious zero addresses
      /\b(0x)?[0-9a-f]{40}\b/i, // EVM addresses in Solana context
    ];
  ...
  validateAddress(address: string): SecurityCheck {
    ...
    if (!/^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(address)) {
      return { passed: false, severity: "critical", message: "Invalid Solana address format" };
    }
    ...
    for (const pattern of this.suspiciousPatterns) {
      if (pattern.test(address)) {
        return { passed: false, severity: "warn", message: "Suspicious address pattern" };
      }
    }
